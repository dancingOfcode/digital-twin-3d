...
const initViewer = () => {
  ...
  moveMesh.position.set(12, 15, -106)
  ...
  viewer.value.scene.add(group1, group2)
  ...
  let mesh = viewer.value.scene.getObjectByName('textureData_9-3')
}
...
const action9 = () => {
  ...
  // animation
  let action9_1_agv = viewer.value.addTweenAnimate('action9_1_agv', agv.position, { x: -412, y: 6, z: 142 }, 8000, 5000, () => {
  ...
  let action9_1_moveBox = viewer.value.addTweenAnimate('action9_1_moveBox', moveBox1.position, { x: -420, y: 12, z: 150 }, 8000, 5000)
  action9_1_agv.onComplete(() => {
    agv.rotation.set(0, Math.PI * 3 / 4, 0)
    let action9_2_agv = viewer.value.addTweenAnimate('action9_2_agv', agv.position, { x: -445, y: 6, z: 109 }, 3000)
    action9_2_agv.onComplete(() => {
      let action9_3_agv = viewer.value.addTweenAnimate('action9_3_agv', agv.position, { x: -445, y: 142, z: 109 }, 3000)
      action9_3_agv.onComplete(() => {
        let action9_4_agv = viewer.value.addTweenAnimate('action9_4_agv', agv.position, { x: -412, y: 142, z: 142 }, 3000)
        action9_4_agv.onComplete(() => {
          agv.rotation.set(0, -Math.PI * 3 / 4, 0)
          let action9_5_agv = viewer.value.addTweenAnimate('action9_5_agv', agv.position, { x: -595, y: 142, z: 319 }, 5000)
          action9_5_agv.onComplete(() => {
            agv.rotation.set(0, -Math.PI / 4, 0)
            viewer.value.addTweenAnimate('action9_6_agv', agv.position, { x: -489, y: 142, z: 428 }, 5000)
          })
        })
      })
    })
  ...
  action9_1_moveBox.onComplete(() => {
    ...
    let action9_2_moveBox = viewer.value.addTweenAnimate('action9_2_moveBox', moveBox1.position, { x: -445, y: 12, z: 109 }, 3000)
    action9_2_moveBox.onComplete(() => {
      let action9_3_moveBox = viewer.value.addTweenAnimate('action9_3_moveBox', moveBox1.position, { x: -445, y: 154, z: 109 }, 3000)
      action9_3_moveBox.onComplete(() => {
        let action9_4_moveBox = viewer.value.addTweenAnimate('action9_4_moveBox', moveBox1.position, { x: -422, y: 154, z: 132 }, 3000)
        action9_4_moveBox.onComplete(() => {
          moveBox1.position.set(-420, 154, 150)
          let action9_5_moveBox = viewer.value.addTweenAnimate('action9_5_moveBox', moveBox1.position, { x: -602, y: 154, z: 326 }, 5000)
          action9_5_moveBox.onComplete(() => {
            moveBox1.position.set(-587, 154, 326)
            viewer.value.addTweenAnimate('action9_6_moveBox', moveBox1.position, { x: -480, y: 154, z: 437 }, 5000).onComplete(() => {
              setTimeout(() => {
                // 清除 action9 相关动画
                let tweenKeys = ['action9_1_agv', 'action9_2_agv', 'action9_3_agv', 'action9_4_agv', 'action9_5_agv', 'action9_6_agv', 'action9_1_moveBox', 'action9_2_moveBox', 'action9_3_moveBox', 'action9_4_moveBox', 'action9_5_moveBox', 'action9_6_moveBox']
                viewer.value.removeTweenAnimate(tweenKeys)
                // 清楚 action9 动画路线
                viewer.value.removeModel(textureData_9.map(item => item.textureKey))
                // agv、物体恢复原位
