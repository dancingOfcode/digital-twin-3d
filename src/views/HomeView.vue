...
const initViewer = () => {
  ...
  modelLoader.loadModelToScene(
    ...
    gltf => {
      gltf.setScale(30)
      worker.value = gltf
      gltf.scene.position.set(50, 0, -145)
...
const action3 = () => {
  ...
  moveBox.position.set(-295, 36.6, 112)
  ...
  // animation
  let action3_1_forklift = viewer.value.addTweenAnimate('action3_1_forklift', forklift.position, { x: -200, y: 22, z: 10 }, 3000, 5000, 5000)
  let action3_1_moveBox = viewer.value.addTweenAnimate('action3_1_moveBox', moveBox.position, { x: -200, y: 36.6, z: 10 }, 3000, 5000, 5000)
  let action3_1_worker = viewer.value.addTweenAnimate('action3_1_worker', worker.value.scene.position, { x: -255, y: 0, z: 70 }, 5000, 5000)
  action3_1_forklift.onComplete(() => {
    forklift.rotation.set(0, Math.PI * 3 / 4, 0)
    forklift.position.set(-200, 22, 52)
    let action3_2_forklift = viewer.value.addTweenAnimate('action3_2_forklift', forklift.position, { x: -102, y: 22, z: 150 }, 6000)
  })
  action3_1_moveBox.onComplete(() => {
    moveBox.position.set(-200, 36.6, 52)
    let action3_2_moveBox = viewer.value.addTweenAnimate('action3_2_moveBox', moveBox.position, { x: -102, y: 36.6, z: 150 }, 6000)
  })
  action3_1_worker.onComplete(() => {
    worker.value.scene.rotation.set(0, Math.PI / 2, 0)
    let action3_2_worker = viewer.value.addTweenAnimate('action3_2_worker', worker.value.scene.position, { x: -120, y: 0, z: 175 }, 6000)
    action3_2_worker.onComplete(() => {
      setTimeout(() => {
        // 清除 action9 相关动画
        let tweenKeys = [
          'action1_1_forklift',
        ...
        viewer.value.removeModel(
          textureData_3.map(item => item.textureKey),
        )
        viewer.value.tweenAnimateList = []
        // forklift、moveBox、worker恢复原位
        forklift.position.set(142, 22, -238)
        forklift.rotation.set(0, Math.PI / 4, 0)
        moveBox.rotation.set(0, -Math.PI / 4, 0)
        moveBox.position.set(12, 15, -106)
        worker.value.scene.position.set(50, 0, -145)
        worker.value.scene.rotation.set(0, -Math.PI / 4, 0)
        // 相机位置
...
const action9 = () => {
  ...
  moveBox.position.set(108, 12, -378)
  // animation
  let action9_1_agv = viewer.value.addTweenAnimate(
    ...
    () => {
      ...
      // 动态改变相机位置
      viewer.value.camera.position.set(pos.x * 1.28, pos.y * 2.5, pos.z * 1.28)
      // 动态计算相机视线
    ...
    action9_2_agv.onComplete(() => {
      let action9_3_agv = viewer.value.addTweenAnimate(
        ...
        () => {
          ...
          agv.getWorldPosition(pos) //获取三维场景中某个对象世界坐标
          // 动态改变相机位置
          viewer.value.camera.position.set(pos.x * 2.2, pos.y * 2, pos.z * 2)
          // 动态计算相机视线
          viewer.value.camera.lookAt(pos.x, pos.y, pos.z)
        }
      )
  ...
  action9_1_moveBox.onComplete(() => {
    moveBox.position.set(-422, 12, 132)
    ...
    action9_2_moveBox.onComplete(() => {
      ...
      action9_3_moveBox.onComplete(() => {
        ...
        action9_4_moveBox.onComplete(() => {
          moveBox.position.set(-420, 154, 150)
          ...
          action9_5_moveBox.onComplete(() => {
            moveBox.position.set(-587, 154, 326)
            viewer.value
              .addTweenAnimate(
                'action9_6_moveBox',
                moveBox.position,
